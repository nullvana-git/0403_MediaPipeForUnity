...
1. 위 링크에서 `com.github.homuler.mediapipe-0.16.3.tgz` 다운로드
2. Unity 메뉴 → **Window → Package Manager**
3. 좌상단 **+** → **Add package from tarball** 선택
4. 다운로드한 `.tgz` 파일 선택

### 3. 모델 파일 배치

MediaPipe 손 랜드마크 모델(`hand_landmarker.bytes`)을 `Assets/StreamingAssets/` 폴더에 넣어주세요.  
플러그인 설치 후 샘플 씬을 참고하거나 [MediaPipe 공식 모델 페이지](https://developers.google.com/mediapipe/solutions/vision/hand_landmarker)에서 다운로드할 수 있습니다.
