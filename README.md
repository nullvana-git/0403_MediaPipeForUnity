...
- **Unity 6000.3.12f1** (다른 버전에서는 UGUI 호환 오류가 발생할 수 있습니다)
- [MediaPipe Unity Plugin v0.16.3](https://github.com/homuler/MediaPipeUnityPlugin/releases/tag/v0.16.3)

### 2. 플러그인 설치

이 저장소에는 플러그인 파일이 포함되어 있지 않습니다. **반드시 `.tgz` 방식으로 설치해야 합니다.** (`.unitypackage` 방식은 UGUI 충돌 오류가 발생합니다)

1. 위 링크 Releases 탭에서 `com.github.homuler.mediapipe-0.16.3.tgz` 다운로드
2. Unity 메뉴 → **Window → Package Manager**
3. 좌상단 **+** → **Add package from tarball** → `.tgz` 파일 선택

### 3. 샘플 임포트

Package Manager에서 `MediaPipe Unity Plugin` 선택 → **Samples 탭** → **Official Solutions → Import**
